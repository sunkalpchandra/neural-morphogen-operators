...
def test_dispersion_relation_is_negative_at_k_zero_for_a_stable_operator():
    ...
    C = 5
    op = ReactionDiffusionOperator(DynamicsConfig(
        channels=C, dt=0.05, n_steps=1, laplacian="spectral")).eval()
    z_ref = torch.randn(1, C, 8, 8) * 0.3
    rep = op.linear_stability(z_ref, k_values=np.array([0.0, 5.0, 20.0]))

    z_star = op._homogeneous_state(z_ref).detach()
    J = op.reaction.jacobian_at(z_star).detach().cpu().numpy()
    expected = float(np.max(np.real(np.linalg.eigvals(J))))
    assert rep["growth_rate"][0] == pytest.approx(expected, abs=1e-5), (
        f"k=0 growth {rep['growth_rate'][0]} != max Re eig(J) {expected}")
