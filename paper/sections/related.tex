...
than a perturbable operator. A separate line imputes unmeasured expression by
transferring it from dissociated reference data --- Tangram
\citep{biancalani2021tangram}, SpaGE \citep{abdelaal2020spage} --- which solves
a different problem than predicting at coordinates a section never sampled. We
include them because they are what a practitioner reaching for missing expression
would actually use, not because they were built for this task.

\textbf{Reaction--diffusion formulations} supply our vocabulary
\citep{turing1952,kondo2010reaction,green2015morphogen} and have been used to
design and analyze graph networks, where the diffusive term drives over-smoothing
and a reactive term counteracts it \citep{grand2021,gread2022}. Our results bear
on that line: the same trade-off appears when the diffusion is spatial rather
than on a graph, and Section~\ref{sec:spectral} bounds how far a loss term can
correct it. RNA velocity \citep{bergen2020scvelo} and perturbation models
\citep{lotfollahi2019scgen,roohani2024gears} model dynamics or interventions
without an explicit spatial operator, which is the gap we address.
