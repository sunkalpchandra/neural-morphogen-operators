...
def main() -> int:
    ...
    done = {(r["spectral_weight"], r["seed"]) for r in rows if not r.get("failed")}

    todo = [(w, s) for w in a.weights for s in a.seeds if (w, s) not in done]
    ...
    for k, (weight, seed) in enumerate(todo, 1):
        try:
            ...
            lg = ExperimentLogger(out / "runs" / f"w{weight}__s{seed}", cfg.to_dict())
            tr = Trainer(model, sec, TrainConfig(**{**cfg.train.to_dict(), "seed": seed,
                                                    "epochs": a.epochs}),
                         lw, lg, device, is_nmo=True)
            res = tr.fit()["test"]
            rows.append(dict(spectral_weight=float(weight), seed=seed, section=a.section,
                             **{m: float(res[m]) for m in METRICS if m in res}))
            print(f"[{k}/{len(todo)}] w={weight:<7} s{seed}  "
            ...
            rows.append(dict(spectral_weight=float(weight), seed=seed, failed=True,
                             error=str(exc)[:160]))
    ...
    if ok:
        import pandas as pd
        df = pd.DataFrame(ok).groupby("spectral_weight")[
